...
trans_cifar100_val = transforms.Compose([transforms.ToTensor(),
                                         transforms.Normalize(mean=[0.507, 0.487, 0.441],
                                                              std=[0.267, 0.256, 0.276])])
trans_cifar10_notreans = transforms.Compose([transforms.ToTensor()])


def get_data(args):
    # cifar10 = datasets.CIFAR10('data/cifar10', train=True, download=True, transform=trans_cifar10_notreans)
    if args.dataset == 'mnist':
        ...
        dict_users_train, rand_set_all = noniid(dataset_train, args.num_users, args.shard_per_user,
                                                args.num_classes)  # train,50,2,10
        ...
        dataset_train = datasets.CIFAR10('data/cifar10', train=True, download=True,
                                         transform=trans_cifar10_notreans)  # trans_cifar10_train
...
def get_model(args):
    ...
    elif args.model == 'mlp' and 'fashionmnist' in args.dataset:
        net_glob = MLP(dim_in=784, dim_hidden=256, dim_out=args.num_classes).to(args.device)
