...
def process_x(raw_x_batch, indd=None):
    """converts string of tokens to array of their indices in the embedding""" #将标记字符串转换为其嵌入中的索引数组
    if indd is None:
...
def process_y(raw_y_batch, indd=None):
    """converts vector of labels to array whose rows are one-hot label vectors  将标签向量转换为行是单热标签向量的数组"""
