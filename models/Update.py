...
class LocalUpdate(object):
    ...
    def train(self, net, w_glob_keys, w_local_keys, last=False, dataset_test=None, ind=-1, idx=-1, lr=0.1):
        bias_p=[]
        weight_p=[]
        for name, p in net.named_parameters():
            if 'bias' in name:
                ...
                weight_p += [p]#是列表的append                 weight_decay， 权重衰减
        optimizer = torch.optim.SGD(
        [     
            {'params': weight_p, 'weight_decay':0.0001},
            {'params': bias_p, 'weight_decay':0}
        ...
        for iter in range(local_eps):
            ...
            batch_loss = []
            for batch_idx, (images, labels) in enumerate(self.ldr_train):#50
                if 'sent140' in self.args.dataset:
                    input_data, target_data = process_x(images, self.indd), process_y(labels,self.indd)
                    if self.args.local_bs != 1 and input_data.shape[0] != self.args.local_bs:
                        break
                    ...
                    images, labels = images.to(self.args.device), labels.to(self.args.device)#一次10个
                    #print("image",len(images))
                    #print("lables",len(labels))
                    net.zero_grad()
                    log_probs ,z= net(images)
                    ...
                    for p in range(len(glob_param)):
                        ...
                        glob_param[p]=glob_param[p].sub(local_param[p])
                        l += torch.norm(glob_param[p])
                    #print("l:",l)
                    l = torch.div(l, l1+l2)
                    
                    l = torch.exp(-l)
                    loss = self.loss_func(log_probs, labels)+1 * l+1 * z
                    loss.backward()
                    optimizer.step()#梯度下降

                num_updates += 1        #last = 250
                batch_loss.append(loss.item())
                if num_updates == self.args.local_updates:
                    done = True
                    break
