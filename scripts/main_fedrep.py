...
if __name__ == '__main__':
    ...
    if 'cifar' in args.dataset or args.dataset == 'mnist' or args.dataset == 'fashionmnist':
        dataset_train, dataset_test, dict_users_train, dict_users_test = get_data(args)
        print(len(dataset_train))
        print(len(dict_users_train[1]))
        print(len(dict_users_train[2]))
    ...
    for iter in range(args.epochs+1):#每一轮
        w_glob = {}
        loss_locals = []
        m = max(int(args.frac * args.num_users), 10)
        ...
        # get weighted average for global weights  加权平均
